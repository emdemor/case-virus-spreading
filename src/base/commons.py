...
def get_last_git_tag() -> str:
    ...
    try:
        ...
        logging.warning(
            "IndexError: No git tags found. You can add one through `git tag <tag_name>`. "
            + "Without git tags, the model version will be set to None."
        )

    except InvalidGitRepositoryError as err:
        logging.warning(
            f"Model version will be set to None.\n"
            + f"InvalidGitRepositoryError: This folder is not a Git repository."
        )
...
def read_csv(filepath: str, *args, **kwargs) -> pd.DataFrame:
    """Read a csv file and converts it in a pandas dataframe.
    ...
    -------
    pd.DataFrame
        Dataframe generated by a tabular structured data from a csv file.
    """
    ...
    try:
        ...
        logging.error(
            f"It was not possible to import {filepath}. "
            + "Exception: "
            + err.__str__()
        )
        raise err
